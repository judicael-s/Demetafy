...
export function DownloadsDock(): JSX.Element {
  ...
  return (
    <Show when={counts().total > 0}>
      <div class="fixed bottom-4 right-4 z-50 w-80 overflow-hidden rounded-xl border border-border bg-surface shadow-xl">
        <div class="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <div class="ml-auto flex items-center gap-1">
            <Show when={counts().active > 0}>
              <button
                class="rounded px-2 py-1 text-xs font-medium text-rose-400 hover:opacity-80"
            ...
            <Show when={retryable().length > 0}>
              <button
                class="rounded px-2 py-1 text-xs font-medium text-accent hover:opacity-80"
                onClick={retryFailed}
              >
            ...
            <Show when={counts().active === 0}>
              <button
                class="rounded px-2 py-1 text-xs text-muted hover:text-ink"
